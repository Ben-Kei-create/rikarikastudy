...
1. Vercel の対象プロジェクトで `Settings → Git` を開く
2. GitHub リポジトリが接続済みで、Production Branch が `main` になっているか確認
3. GitHub へ push 後、`Deployments` に最新コミットSHAで自動ビルドが作成されるか確認
4. 失敗している場合は `Deployments` のログで環境変数不足やビルドエラーを確認

> 反映タイミングの目安:
> - 通常は push 後 **30秒〜3分程度** で新しいデプロイが作成されます。
> - `Ready` になっても端末側キャッシュで古い画面が見えることがあるため、ハードリロード（`Ctrl/Cmd + Shift + R`）で確認してください。
> - すぐ反映されない場合は Vercel ダッシュボードの `Deployments` で進行中 / 失敗状態を先に確認するのが確実です。
